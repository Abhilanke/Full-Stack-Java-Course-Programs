...
public class With_stream_Api {
    ...
    public static void main(String[] args) {
        ...
        // l1.stream().filter((v)-> v.price > 30000).forEach((h)-> System.out.println(h));
            //Or
           ...
           List<Vehical1> l2   = l1.stream().
                                 filter((v)-> v.price > 30000).
         ...
         System.out.println();
       Map<String,Double> map1   = l1.stream().filter((v)-> v.price > 30000).collect(Collectors.toMap(key-> key.name,val-> val.price));
         System.out.println(map1);
         
         //map()
       Long count = l1.stream().filter((vh)-> vh.price > 30000).count();
         System.out.println("no of records returned by the filter method = "+count);
    }
